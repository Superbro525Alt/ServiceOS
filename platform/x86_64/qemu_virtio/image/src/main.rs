...
use serviceos_kernel_core::{
    Kernel, rng as kernel_rng, syscall, task as kernel_task, user as kernel_user,
};
use serviceos_platform_qemu_virtio::{
    audio, block, boot, display, gpu, input, network, rng, serial, sound,
};
...
fn kernel_main() -> Status {
    ...
    let rng_summary = kernel_rng::initialize(rng::initialize());
    log(
        "rng",
        format_args!(
    ...
    if let Some(summary) = network::bringup_summary() {
        match summary.interrupt {
            network::NetworkInterruptModel::Msix {
                queue_vector,
                config_vector,
            } => match config_vector {
                Some(config_vector) => log(
                    "network",
                    format_args!(
                        "backend={:?} pci={:02x}:{:02x}.{} irq=msi-x vector={:#04x} config-vector={:#04x} mac={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} mtu={}",
                        summary.backend,
                        summary.pci_bus,
                        summary.pci_device,
                ...
                None => log(
                    "network",
                    format_args!(
                        "backend={:?} pci={:02x}:{:02x}.{} irq=msi-x vector={:#04x} config-vector=none mac={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} mtu={}",
                        summary.backend,
                        summary.pci_bus,
                        summary.pci_device,
                        summary.pci_function,
