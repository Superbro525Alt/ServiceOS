...
use rt::{
    LogEvent, LogSeverity, PermissionPolicyState, RawMessage, RuntimeRunState, RuntimeStatus, RuntimeTag,
    SecurityAuditKind,
    RuntimeWorkloadKind, TaskStateCode,
};

use crate::{
    consts::{MAX_AUDIT, MAX_ENVS, MAX_RUNS, MAX_STORAGE_PATH},
    types::{AuditSlot, EnvSlot, FixedBytes, Profile, RunSlot},
    util::{emit_log, instantiate_env, pack_pair, release_run_slot, resolve_guest_path, sensitive_capabilities},
};
...
fn handle_env_decision_request(
    log_handle: rt::Handle,
    envs: &mut [EnvSlot; MAX_ENVS],
    audits: &mut [AuditSlot; MAX_AUDIT],
    next_audit_sequence: &mut u32,
