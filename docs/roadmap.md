...
| &#x2B1C; | Desktop polish | Partial: broader global action routing landed in the command palette — six system actions (toggle media/volume overlay, toggle notification history, toggle clipboard history, dismiss-all notifications, cycle Settings pages of a running settings app through the existing app-control key contract, and a lock-session shell stub that tears down overlays) rank alongside app launches via the shared prefix/match/recency scorer, with host unit tests for the apps-plus-actions ranking merge and per-action query reachability; cross-app interaction patterns remain open |
| &#x2B1C; | Desktop polish | Partial: the clipboard history panel is now interactive — pointer clicks (and Enter) activate a history row through the existing clipboard-service `Activate` contract, promoting it to the current clipboard and forwarding Ctrl+V down/up to the focused app so terminals paste immediately, with unavailable-service and out-of-range rows degrading to notices instead of errors; drag-and-drop, desktop gestures, hot corners, and richer file handoff remain open |
