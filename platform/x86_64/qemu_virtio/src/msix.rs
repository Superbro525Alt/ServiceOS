...
//!   5. point every virtio queue at the caller's MSI-X vector in the common
//!      config (the config-change vector stays NO_VECTOR: config events are
//!      not delivered — documented v0 limitation)
//!   6. unmask entry 0
...
/// events stay disabled for v0; see the module comment).
...
pub(crate) enum MsixOutcome {
    ...
    /// MSI-X programmed: the device signals on this vector number until the
...
/// Configure the virtio device for MSI-X delivery on `vector` (an arch MSI
/// vector, `MSI_VECTOR_BASE + slot`) and return [`MsixOutcome::Ready`] with
/// that vector, or the reason MSI-X was skipped.
pub(crate) fn try_setup_msix(
    ...
    // delivery, edge trigger, on the caller's arch MSI vector.
    let entry = msi::MsixTableEntry::new_edge_fixed(0, vector, true);
    ...
    // Assign all virtio queues to the caller's MSI-X vector (config + queues
    // share one vector for v0).
