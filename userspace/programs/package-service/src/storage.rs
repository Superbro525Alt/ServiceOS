...
pub(crate) const TRUST_ROOTS_PATH: &str = "state/packages/trust-roots.cfg";
...
pub(crate) fn persist_trust_roots(storage_handle: rt::Handle) -> rt::Result<()> {
    let mut text = rt::FixedLogBuffer::<MAX_STATE_BYTES>::new();
    crate::signing::serialize_trust_roots(unsafe { &*core::ptr::addr_of!(TRUST_ROOTS) }, &mut text);
    write_storage_file(storage_handle, TRUST_ROOTS_PATH, text.as_bytes())
}

pub(crate) fn load_trust_roots(storage_handle: rt::Handle) -> rt::Result<()> {
    let (blob, len) = match rt::storage_open(storage_handle, TRUST_ROOTS_PATH) {
        Ok(value) => value,
        Err(rt::Error::NotFound) => return Ok(()),
        Err(error) => return Err(error),
    };
    ...
    let _ = rt::storage_blob_close(blob);
    let text = core::str::from_utf8(&bytes[..loaded]).map_err(|_| rt::Error::InvalidArgument)?;
    let parsed = crate::signing::parse_trust_roots(text);
    unsafe {
        TRUST_ROOTS = parsed;
    }
    Ok(())
