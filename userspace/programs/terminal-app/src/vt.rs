...
fn finish_osc(tab: &mut TerminalTab) {
    if let Some(separator) = tab.osc_bytes[..tab.osc_len].iter().position(|byte| *byte == b';') {
...
fn apply_csi(tab: &mut TerminalTab, columns: usize, tab_index: usize, opcode: u8) {
    match opcode {
        ...
        b'B' => tab.cursor_line = (tab.cursor_line + csi_param(tab, 0, 1)).min(tab.line_count.saturating_sub(1)),
        b'C' => tab.cursor_col = (tab.cursor_col + csi_param(tab, 0, 1)).min(columns.saturating_sub(1)),
        b'D' => tab.cursor_col = tab.cursor_col.saturating_sub(csi_param(tab, 0, 1)),
        b'G' => tab.cursor_col = csi_param(tab, 0, 1).saturating_sub(1).min(columns.saturating_sub(1)),
        b'H' => {
            tab.cursor_line = csi_param(tab, 0, 1).saturating_sub(1).min(MAX_SCROLLBACK_LINES.saturating_sub(1));
            tab.cursor_col = csi_param(tab, 1, 1).saturating_sub(1).min(columns.saturating_sub(1));
        }
...
fn csi_param(tab: &TerminalTab, index: usize, default: usize) -> usize {
    tab.csi_params.get(index).copied().unwrap_or(default).max(default)
}
...
fn advance_line(tab: &mut TerminalTab, tab_index: usize, wrapped: bool) {
    unsafe { WRAPS.tab_mut(tab_index)[tab.cursor_line] = wrapped; }
...
fn delete_chars(tab: &mut TerminalTab, columns: usize, tab_index: usize, count: usize) {
    unsafe {
        ...
        row.copy_within((tab.cursor_col + count).min(columns)..columns, tab.cursor_col);
        row[columns.saturating_sub(count.min(columns - tab.cursor_col))..columns].fill(Cell::blank());
    }
}
...
fn insert_lines(tab: &mut TerminalTab, tab_index: usize, count: usize) {
    unsafe {
        ...
        grid.copy_within(tab.cursor_line..MAX_SCROLLBACK_LINES - shift, tab.cursor_line + shift);
        wraps.copy_within(tab.cursor_line..MAX_SCROLLBACK_LINES - shift, tab.cursor_line + shift);
...
fn delete_lines(tab: &mut TerminalTab, tab_index: usize, count: usize) {
    unsafe {
        ...
        grid.copy_within(tab.cursor_line + shift..MAX_SCROLLBACK_LINES, tab.cursor_line);
        wraps.copy_within(tab.cursor_line + shift..MAX_SCROLLBACK_LINES, tab.cursor_line);
...
pub(crate) fn reflow_tab(tab: &mut TerminalTab, tab_index: usize, old_columns: usize, new_columns: usize) {
